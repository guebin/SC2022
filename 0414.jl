...
- 결국 $f_Z(x)$가 분포C의 가능한 pdf중 하나의 형태일때 $\frac{1}{b}f_Z(\frac{x-a}{b})$ 역시 분포C의 가능한 pdf 중 하나의 형태라면 분포C는 위치모수와 척도모수를 가진다. 
-  $f_Z(x)$가 분포A의 가능한 pdf중 하나의 형태일때 $f_Z(x-a)$ 역시 분포A의 가능한 pdf 중 하나의 형태라면 분포A는 위치모수를 가진다. 
-  $f_Z(x)$가 분포B의 가능한 pdf중 하나의 형태일때 $\frac{1}{b}f_Z(\frac{x}{b})$ 역시 분포B의 가능한 pdf 중 하나의 형태라면 분포B는 척도모수를 가진다. 
"""
...
이다. 그런데 이것은 평균이 3이고 분산이 1인 정규분포의 pdf와 모양이 같다. 일반화하면 
