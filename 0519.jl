...
let 
	U1,d1,V1 = svd(X'X) 
	V2 = [V1[:,1] -V1[:,2] V1[:,3]] # 두번쨰 컬럼의 부호를 임의로 바꿔버렸음 
...
let 
	...
	Z = X * V1[:,1:2] # 부호를 안바꾼 V1으로 Z를 구함 
	Z2 = X * V2[:,1:2] # 부호를 바꾼 V2으로 Z를 구함 
	[Z Z2]	# 비교해보면 당연히 Z와 Z2는 다르다 
	X̂ = Z * V1[:,1:2]'
	X̂2 = Z2 * V2[:,1:2]' 
	[X̂ X̂2] # 그런데 reconstruction은 같다. (왜?) <-- 다음에 풀게요 
end 
...
md"""
`-` Z2를 이용하여 "scatter(Z2[:,1],Z2[:,2],markersize=0.2)"의 그림을 그려보라. 두 개의 그룹이 여전히 잘 구분되는지 확인하라.
"""
